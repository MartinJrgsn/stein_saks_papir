use game::{error::PromptError, player::PlayerObj, ui::{TUIObj, UIObj}};
...
pub trait UIRps: UIObj
{
    fn promt_for_name(self: &mut Self, is_valid: Option<&dyn Fn(&str) -> Option<NameError>>) -> Result<String, PromptError>;
    fn promt_for_choice(self: &mut Self, player: &HumanRps, session: &SessionRps, choice_log: &[[Choice; 2]]) -> Result<Option<PlayerDecision>, PlayerDecisionError>;
...
impl UIRps for TUI
{
    ...
    fn promt_for_choice(self: &mut Self, player: &HumanRps, session: &SessionRps, choice_log: &[[Choice; 2]]) -> Result<Option<PlayerDecision>, PlayerDecisionError>
    {
        let players = session.get_players();

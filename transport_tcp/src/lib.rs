...
mod test
{
    ...
    fn test_stream_chat()
    {
        ...
        #[allow(unused)]
        const SIGURD: IpAddr = IpAddr::V4(Ipv4Addr::new(192, 168, 86, 25));

        let target_ip = local_ip_address::local_ip().unwrap();
        ...
        let stream = loop
        {
            ...
            {
                Ok(stream) => break Arc::new(RwLock::new(stream)),
                Err(error) => {
        ...
        std::thread::spawn(move || loop
        {
            ...
            if let Some(message) = stream2.read().unwrap().receive().unwrap()
            {
        ...
        loop
        {
            ...
            if size != 0
            {
                stream.read().unwrap().send(keyboard_input).unwrap()
            }
        }
    }
}
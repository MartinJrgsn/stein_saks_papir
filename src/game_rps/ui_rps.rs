...
pub trait UIRps: UI
{
    ...
    fn promt_for_choice(self: &mut Self, player: &HumanRps, actor: &ActorServer<2>, choice_log: &[[Choice; 2]]) -> Result<Option<PlayerDecision>, PlayerDecisionError>;
}
...
impl UIRps for TUI
{
    ...
    fn promt_for_choice(self: &mut Self, player: &HumanRps, actor: &ActorServer<2>, choice_log: &[[Choice; 2]]) -> Result<Option<PlayerDecision>, PlayerDecisionError>
    {
        let (players, _) = actor.get_players_or_wait()?;
